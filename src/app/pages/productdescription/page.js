    ...
    <div className="bg-white border border-gray-200 rounded-2xl shadow-lg p-6">
      ...
      <div className="flex flex-col lg:flex-row gap-6">
          ...
          {product.images.map((image, index) => (
            ...
            >
              <Image
                ...
                selectedImageIndex === index ? "border-2 border-primary" : "border-transparent"
              }`}
              />
            </div>
          ))}
        ...
        {isMagnifierVisible && (
          <div className="lg:w-2/5 w-full">
            <div
              className="w-full h-[350px] border-2 border-gray-300 overflow-hidden rounded-lg"
              style={{
          ...
          <div className="flex space-x-4">
            {product.color.map((color, index) => (
              <div
                ...
                className={`w-10 h-10 cursor-pointer rounded-full flex items-center justify-center transition-all duration-300 relative border-2 ${
                  selectedColor === color.code ? "border-primary" : "border-gray-300"
                }`}
                style={{
      ...
      <div className="flex flex-col items-center gap-4 mt-6">
        ...
        <div className="flex items-center space-x-3">
          ...
          <span className="text-lg font-medium">{wholesaleQuantity}</span>
          <button
            className="w-10 h-10 bg-gray-200 rounded-full flex items-center justify-center text-gray-600 hover:bg-gray-300 transition"
      ...
      {showToast && (
        <div className="fixed inset-0 flex justify-center items-center z-50">
          <div className="bg-teal-500 text-white py-3 px-6 rounded-lg shadow-lg flex items-center">
      ...
      <h2 className="text-xl font-bold mt-8">{product.title}</h2>
      <div className="mt-4">
        <h2 className="text-lg font-bold">Product Overview</h2>
        <p className="text-sm text-gray-700">{product.productOverview}</p>
      </div>
          ...
          <tbody>
            {product.technicalSpecs.split("\n").map((info, index) => (
              <tr key={index} className="border-b border-gray-200">
                <td className="py-2 px-4 text-sm text-gray-700">{info}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {/* Why Buy Section */}
      <div className="mt-6">
        <p className="text-sm font-medium"><strong>{product.whyBuy.split('?')[0]}?</strong></p>
        <p className="text-sm text-gray-700">{product.whyBuy.split('?').slice(1).join('?')}</p>
      </div>
    </div>
    <RelatedProducts />
  </div>
