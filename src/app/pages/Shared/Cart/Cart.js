    ...
    >
      ...
      <div className="w-full h-full sm:w-80 bg-white shadow-xl p-4 overflow-y-auto transform transition-all duration-300 ease-out z-50">
        <div className="flex justify-between items-center mb-4">
          <span className="font-semibold text-sm text-black">{cartItems.length} ITEMS</span>
          <button
            onClick={toggleCart}
            className="bg-gray-200 hover:bg-gray-300 text-black hover:text-black transition duration-300 text-lg rounded-full p-4"
          >
            <FaTimes />
          </button>
        </div>
        ...
        <div className="space-y-3 max-h-[70vh] overflow-y-auto">
          {cartItems.length === 0 ? (
            <div className="flex flex-col justify-center items-center space-y-2 " style={{ marginTop: "80%" }}>
              ...
              <p className="text-sm text-black ">Your cart is empty. Add some items!</p>
            </div>
          ) : (
            cartItems.map((item) => (
              <div
                key={item.cartId}
                className="flex justify-between items-center p-3 bg-gray-200 rounded-lg shadow-md hover:scale-105 transform transition duration-300 ease-in-out"
              >
                <div className="flex items-center space-x-3">
                  <div className="flex items-center space-x-1">
                    <button
                      className="bg-gray-600 text-black p-1 rounded-full hover:bg-gray-500 transition duration-300"
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-black">৳{(item.price * item.quantity).toFixed(0)}</p>

                    {/* Show color swatch */}
                    {item.color && item.color.length > 0 && (
                      <div className="flex items-center space-x-2">
                        <span className="text-xs text-black">Color:</span>
                        <div
        ...
        <div className="mt-4">
          <button
            className="w-full py-2 bg-gradient-to-r from-yellow-400 to-yellow-500 text-black font-bold text-sm rounded-lg hover:scale-105 transition duration-300 ease-in-out disabled:bg-gray-600"
