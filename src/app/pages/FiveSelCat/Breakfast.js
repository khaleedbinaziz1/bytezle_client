...
import bannerImage from './Banner-Breakfast.png';
...
const Breakfast = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter products to only include "Breakfast" category and "showProduct" = "On"
        const breakfastItems = response.data.filter(
          (product) => product.category === 'Breakfast' && product.showProduct === "On"
        );
        
        setProducts(breakfastItems);
        setTotalPages(Math.ceil(breakfastItems.length / itemsPerPage));
      } catch (error) {
  ...
  <button 
    ...
    onClick={() => router.push('/pages/freshdeals?category=Breakfast')}
  >
    See All
  </button>
</div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 mt-10">
            {loading ? (
              ...
              products.slice(startIndex, endIndex).map(product => (
                <div key={product._id} className="card bg-white rounded-lg overflow-hidden shadow-md hover:shadow-xl cursor-pointer mb-6 sm:mb-0"
                  ...
                  <div className="flex justify-center items-center h-32">
                    {product.images && product.images.length > 0 ? (
                      product.images[0]?.img ? (
                        <Image src={`data:${product.images[0].contentType};base64,${product.images[0].img}`} alt={product.name} width='100' height='100' />
                      ) : (
                  ...
                  <div className="p-4">
                    ...
                    {/* EACH Text */}
                    <span className="text-gray-500 text-sm font-extrabold block text-center mb-2">
                      EACH
                    </span>
