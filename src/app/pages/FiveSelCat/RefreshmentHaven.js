...
import bannerImage from './refreshment_heaven_banner.png';
import addToWishlist from '../Wishlist/addToWishlist';

const RefreshmentHaven = () => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        const refreshmentHavenProducts = response.data.filter(
          (product) => product.subcategory === 'Refreshment Haven' && product.showProduct === "On"
        );
  
        setProducts(refreshmentHavenProducts);
        setTotalPages(Math.ceil(refreshmentHavenProducts.length / itemsPerPage));
      } catch (error) {
  ...
  return (
    <div className="mx-auto p-4 pt-5 mt-5 text-center rounded bg-white">
      <div className="text-left mb-5 flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold">Refreshment Haven</h1>
        </div>
        <button 
...
export default RefreshmentHaven;
